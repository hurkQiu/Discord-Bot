...
- `!idiom new`：開始新的猜成語，從內建題庫隨機抽一個四字成語並附上一句釋義提示，共有 6 次機會
- `!idiom <四字成語>`：送出一次猜測，例如 `!idiom 一帆風順`；每個字會標示 🟩（字與位置都對）/ 🟨（成語裡有這個字但位置不對）/ ⬛（成語裡沒有這個字），重複字元也會正確處理
- `!idiom hint`：花一次機會，讓機器人隨機公布一個尚未提示過的字的位置與內容，猜不出來時可以拿來換取線索
- `!idiom show` / `!idiom stop`：重新顯示目前猜測紀錄 / 結束目前的猜成語（會公布答案）

猜測不會檢查是否為真實成語，任何四個字都能送出；每局一開始就會給釋義提示，`hint` 跟猜測共用同一組 6 次機會的額度；次數用完會自動公布答案。內建題庫（約 120 條）是精選的常見成語與釋義（非完整詞典，釋義為簡化說明，非正式辭典引文）。
